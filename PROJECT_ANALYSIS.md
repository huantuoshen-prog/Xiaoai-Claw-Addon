...
- `install.sh` 必须在仓库和 release 包里保留可执行位
- 如果某些解压工具把执行位丢了，用户仍然可以直接运行 `bash ./install.sh`
...
### 0.2 为什么 `openclawTo` 的细节不该写在 README

对普通用户来说，只要插件能把登录入口、语音转发和通知发送给 OpenClaw 就够了。

但从实现上看，`openclawTo` 是一个比较容易踩坑的配置点：

- 如果手工漏填，插件过去会在语音转发或登录通知时直接报错
- 如果盲目写死，又会在不同用户的 Telegram 账号结构下出问题
- 如果随便自动猜测，多账户场景下又可能把消息发错目标

因此现在的处理策略是：

1. 安装时，如果当前 Telegram 配置里只有一个明确的 `allowFrom` 目标，就自动回写到插件配置。
2. 运行时，如果用户没配、但全局配置里存在唯一可推断目标，就做安全兜底。
...
当前版本已经收敛成以 `pause` 为核心。

原因很直接：

- 实测 pause 最稳定
- stop / 额外闭嘴路径收益不高，复杂度更高

### 15.2 pause 会并发尝试多条底层路径

`pauseSpeaker()` 会：

- 走 MiNA `playerPause`
- 如果该机型有对应 MIOT action，也同时走 MIOT pause
- 按重试延迟数组做多次尝试
- 用 `Promise.any()` 抢最快成功路径

这正是“稳准狠地让小爱闭嘴”的核心策略。

### 15.3 转发和打断并行

`forwardToOpenclaw()` 不会等 pause 成功后再开始。
...
控制页不再只是“高度自适应”，而是通过 JS 计算 `grid-row-end` 做紧凑瀑布布局，避免高度不一致时留下大缝隙。
...
pause 打断和转发给 OpenClaw 并行执行。

这直接减少总等待时间。

### 29.3 优先 pause

实践证明，pause 是当前最稳的快速打断路径。

收敛到 pause 优先，能减少无效分支和试错时间。
...
### 34.5 `speaker/play-audio` 的返回时间不等于真正出声时间

当前稳定版已经把接口返回压到约 `2.5s` 级别，但这不代表音箱一定会在相同时间内真实出声。

原因仍然包括：

- 小米云 `player_play_music` 的受理和设备侧落地延迟
- 设备 `playerGetStatus` 的回读延迟
- 短音频在某些机型上的 `status=2 -> status=1` 抖动

所以以后评估“音频播放变慢了没有”，要分开看：
